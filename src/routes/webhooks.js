...
import { countWebhooks, deleteWebhook, findWebhookByEndpoint, insertWebhook, listWebhooks, updateWebhook } from '../db/webhooks.js';
...
webhooks.get('/', async (c) => {
	...
	try {
		const [total, rows] = await Promise.all([countWebhooks(c.env.DB), listWebhooks(c.env.DB, page)]);
...
webhooks.post('/', async (c) => {
	...
	try {
		const webhook = await insertWebhook(c.env.DB, { ...fields, endpoint: generateEndpoint() });
...
const update = async (c) => {
	...
	try {
		const webhook = await updateWebhook(c.env.DB, id, fields);
		if (!webhook) return errorResponse('Webhook not found', 404);
		return jsonResponse(sanitizeWebhook(webhook));
	} catch (err) {
...
webhooks.delete('/:id', async (c) => {
	...
	try {
		const deleted = await deleteWebhook(c.env.DB, id);
