-- Baseline schema. IF NOT EXISTS keeps this a no-op on databases created from the old schema.sql,
-- so existing deployments can adopt `wrangler d1 migrations apply` without data loss.
...
CREATE INDEX IF NOT EXISTS idx_webhook_stats_webhook_id_date ON webhook_stats(webhook_id, date);
